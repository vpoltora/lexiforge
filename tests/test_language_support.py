"""
Test language support for VocabAI plugin.
...
exec(open(os.path.join(parent_dir, "language_constants.py")).read())
exec(open(os.path.join(parent_dir, "tts_client.py")).read().replace("from . import language_constants", ""))
exec(open(os.path.join(parent_dir, "ai_client.py")).read())
    ...
    for source_lang, word, target_lang in test_cases:
        ...
        try:
            ...
            if definition and example:
                ...
                print(f"  ❌ FAILED - Missing definition or example")
                results.append((source_lang, target_lang, "❌ FAIL"))
                failed.append(f"{source_lang}→{target_lang}")
        except Exception as e:
            print(f"  ❌ ERROR: {e}")
            results.append((source_lang, target_lang, f"❌ ERROR"))
            failed.append(f"{source_lang}→{target_lang}")
...
if __name__ == "__main__":
    print("VocabAI Language Support Testing")
    print("=" * 80)
