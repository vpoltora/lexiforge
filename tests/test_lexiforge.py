...
class TestLexiForge(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if not self.config or not self.config.get("api_key"):
            self.fail(
                "Config not found or API Key missing. Please restore config.json with a valid key."
            )
        self.api_key = self.config["api_key"]
        self.model = self.config.get("model", "gemini-flash-latest")

    def test_generate_content_spanish_to_english(self) -> None:
        ...
        definition, example, base_form = ai_client.generate_content(
            word, source_lang, self.api_key, self.model, definition_lang
        )
        ...
        # Checks
        self.assertTrue(
            base_form.lower() == "gato",
            f"Base form should be 'gato', got '{base_form}'",
        )
        # Definition should be in English (contains English words)
        self.assertTrue(
            any(w in definition.lower() for w in ["cat", "animal", "feline"]),
            "Definition should contain English translation",
        )

        # Example should be in Spanish (source language)
        # Simple check: should contain common Spanish words or the word itself
        self.assertTrue(
            "gato" in example.lower()
            or "el" in example.lower()
            or "un" in example.lower(),
            "Example should be in Spanish",
        )

    def test_generate_content_english_to_russian(self) -> None:
        ...
        self.assertEqual(
            base_form.lower(), "run", f"Base form should be 'run', got '{base_form}'"
        )
        ...
        self.assertTrue(
            any(
                w in example.lower()
                for w in ["run", "running", "ran", "he", "she", "it"]
            ),
            "Example should be in English",
        )

    def test_audio_generation(self) -> None:
        # Test TTS generation
        word = "test"
        source_lang = "English"
